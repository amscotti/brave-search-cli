...
/**
 * Every machine document and line the news presenter emits satisfies the published v1
 * schemas: the success and failure envelopes against their envelope schemas, and each JSONL
 * line — warning, result, summary, error — against the jsonl record schema with its pinned
 * news payloads.
 */
final class NewsPresenterSchemaValidationTest {

    private final JsonMappers mappers = new JsonMappers();

    private final SchemaCatalog schemas = new SchemaCatalog();

    private final NewsSearchPresenterImpl presenter = new NewsSearchPresenterImpl(
            ...
            (mode, diagnostics, results, quiet) -> new io.amscotti.bravesearch.adapter.cli.presentation.ModeAwareWarnings(
                    mode, NewsSearchPresenter.COMMAND, diagnostics, results, new JsonlCodec(mappers), quiet));
    ...
    void zeroResultDocumentsSatisfyTheirSchemas() {
        NewsSearchResult empty = new NewsSearchResult(
    ...
    private String render(Outcome<NewsSearchResult> outcome, OutputRequest output) {
        ByteArrayOutputStream stdout = new ByteArrayOutputStream();
        int exit = presenter.present(
                outcome,
                NewsSearchRequest.builder("three word query").build(),
                output,
    ...
    private static Outcome.Success<NewsSearchResult> success() {
    ...
    private static Outcome.Failure<NewsSearchResult> authenticationFailure() {
    ...
    private static Outcome.Failure<NewsSearchResult> transportFailure() {
    ...
    private static byte[] fixture(String name) {
        String resource = "/fixtures/brave/news/" + name;
        try (java.io.InputStream bytes = NewsPresenterSchemaValidationTest.class.getResourceAsStream(resource)) {
            if (bytes == null) {
