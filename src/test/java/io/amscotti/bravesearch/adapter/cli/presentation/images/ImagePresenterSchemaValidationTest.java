...
/**
 * Every machine document and line the images presenter emits satisfies the published v1
 ...
 * pinned images payloads.
 */
final class ImagePresenterSchemaValidationTest {

    private final JsonMappers mappers = new JsonMappers();

    private final SchemaCatalog schemas = new SchemaCatalog();

    private final ImageSearchPresenterImpl presenter = new ImageSearchPresenterImpl(
            ...
            (mode, diagnostics, results, quiet) -> new io.amscotti.bravesearch.adapter.cli.presentation.ModeAwareWarnings(
                    mode, ImageSearchPresenter.COMMAND, diagnostics, results, new JsonlCodec(mappers), quiet));
    ...
    void zeroResultDocumentsSatisfyTheirSchemas() {
        ImageSearchResult empty = new ImageSearchResult(
    ...
    private String render(Outcome<ImageSearchResult> outcome, OutputRequest output) {
        ByteArrayOutputStream stdout = new ByteArrayOutputStream();
        int exit = presenter.present(
                outcome,
                ImageSearchRequest.builder("three word query").build(),
                output,
    ...
    private static Outcome.Success<ImageSearchResult> success() {
    ...
    private static Outcome.Failure<ImageSearchResult> authenticationFailure() {
    ...
    private static Outcome.Failure<ImageSearchResult> transportFailure() {
    ...
    private static byte[] fixture(String name) {
        String resource = "/fixtures/brave/images/" + name;
        try (java.io.InputStream bytes = ImagePresenterSchemaValidationTest.class.getResourceAsStream(resource)) {
            if (bytes == null) {
