...
/**
 * Every machine document and line the context presenter emits satisfies the published v1
 * schemas: the success and failure envelopes against their envelope schemas, and each
 * JSONL line — result, summary, error — against the jsonl record schema with its pinned
 * context payloads.
 */
final class ContextPresenterSchemaValidationTest {
    ...
    @Test
    void zeroContextDocumentsSatisfyTheirSchemas() {
        ContextResult empty =
                new ContextResult(200, new UpstreamPayload(fixture("zero-context.json")), RateLimitSnapshot.empty(), null, null, null);
        ...
        assertValid(
                ...
                render(new Outcome.Success<>(empty), machine(OutputMode.JSONL, false)));
    }
    ...
    private String render(Outcome<ContextResult> outcome, OutputRequest output) {
        ByteArrayOutputStream stdout = new ByteArrayOutputStream();
        int exit = presenter.present(
                outcome,
                ContextRequest.builder("three word query").build(),
                output,
    ...
    private static Outcome.Success<ContextResult> success() {
        return new Outcome.Success<>(
                new ContextResult(
                        200,
                        new UpstreamPayload(fixture("grounded-context.json")),
    ...
    private static Outcome.Failure<ContextResult> authenticationFailure() {
    ...
    private static Outcome.Failure<ContextResult> transportFailure() {
    ...
    private static byte[] fixture(String name) {
        ...
        try (java.io.InputStream bytes = ContextPresenterSchemaValidationTest.class.getResourceAsStream(resource)) {
            if (bytes == null) {
