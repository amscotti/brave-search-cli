 ...
 */
final class AnswersCompositionTest {
    ...
    void aLoopbackRunResolvesItsCredentialExactlyOnce() {
        ...
        AnswersComposition process = AnswersComposition.process(storedCredentials(), environment, exchanges);

        RunResult result = runMain(exchanges, environment, process, "answers", "--no-stream", "--base-url", "http://127.0.0.1:1", "what is artemis");
    ...
    private RunResult runMain(
            ExchangeRegistry exchanges, Function<String, String> environment, AnswersComposition answersSearch, String... args) {
