 ...
 */
final class NewsSearchCompositionTest {
    ...
    void aLoopbackRunResolvesItsCredentialExactlyOnce() {
        ...
        NewsSearchComposition process = NewsSearchComposition.process(storedCredentials(), environment);

        RunResult result = runMain(process, "news", "--base-url", "http://127.0.0.1:1", "q");
    ...
    private RunResult runMain(NewsSearchComposition newsSearch, String... args) {
        ...
        try {
            int exitCode = new Main(
                            ...
                            config,
                            WebSearchComposition.process(storedCredentials()),
                            ContextComposition.process(storedCredentials()),
                            newsSearch)
                    .execute(args);
