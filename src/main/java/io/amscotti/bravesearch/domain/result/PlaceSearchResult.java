...
/**
 * One completed place search exchange as the application sees it: the HTTP status, the
 * lossless bounded body bytes, and the metadata observed on the response headers.
 *
 * <p>The body stays raw on purpose — the structured projection of places is a separate
 * concern fed from these bytes — so no parsing failure can lose or alter what the
 * upstream sent. {@code rateLimits} is never null (an exchange without rate-limit
