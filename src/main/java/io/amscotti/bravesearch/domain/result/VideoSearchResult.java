...
/**
 * One completed videos search exchange as the application sees it: the HTTP status, the
 * lossless bounded body bytes, and the metadata observed on the response headers.
 *
 * <p>The body stays raw on purpose — the structured projection of video results is a
 ...
 * PagedExchange} — the interface is exactly its member set — so the shared pagination
 * machinery accepts it without adaptation.
 */
