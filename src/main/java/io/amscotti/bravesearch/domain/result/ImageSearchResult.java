...
/**
 * One completed images search exchange as the application sees it: the HTTP status, the
 * lossless bounded body bytes, and the metadata observed on the response headers.
 *
 * <p>The body stays raw on purpose — the structured projection of image results is a
 ...
 * null unless the exchange's response headers offered them. The record implements {@link
 * PagedExchange} — the interface is exactly its member set — so the shared single-request
 * rendering machinery accepts it without adaptation.
 */
