...
/**
 * One parsed place search invocation in the shape the exchange needs: the validated
 * request, the origin the parsed {@code --base-url} selected, the already-resolved
 * credential the preflight drew for that origin, the two parsed budgets, and the parsed
 * {@code Api-Version} pin — {@code null} when none was given.
 *
 * <p>The credential travels inside the invocation so it resolves exactly once per run:
 * the command's preflight is the single resolution, and the exchange consumes the
 * result instead of re-resolving from the environment.
 *
 ...
 */
public record PlaceSearchDispatch(
