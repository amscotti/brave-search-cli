...
import io.amscotti.bravesearch.application.port.out.AnswersPort;
...
/**
 * The Brave HTTP gateway behind {@link AnswersPort}: one assembled POST exchange per
 * answers invocation — today the blocking chat-completions form; the streaming response
 * family is not carried by this gateway yet.
 *
 ...
 * rate-limit snapshot, the Answers usage the {@code X-Request-*} response headers offered,
 * and the request and API version identifiers, with no java.net.http type crossing the
 * port. Failures keep the redacted shape the transport and its classifier already
 * produced. A thread interrupted mid-exchange keeps its interrupt status and reports a
 * TRANSPORT failure, because the port's contract is an outcome, never a checked break.
 * The gateway closes the transport it was handed on {@link #close()}.
 */
public final class BraveHttpAnswersGateway implements AnswersPort, AutoCloseable {
     ...
     */
    public BraveHttpAnswersGateway(
    ...
    @Override
    public Outcome<AnswersResult> answer(AnswersRequest request) {
        ...
        try {
            exchange = transport.send(
                    AnswersEndpoint.assemble(request, origin, credential, pinnedApiVersion), totalTimeout);
    ...
    private static AnswersResult project(BraveHttpResponse response) {
