...
import io.amscotti.bravesearch.application.port.out.ImageSearchPort;
...
/**
 * The Brave HTTP gateway behind {@link ImageSearchPort}: one assembled GET exchange per
 * search.
 *
 ...
 * shape the transport and its classifier already produced. A thread interrupted mid-exchange
 * keeps its interrupt status and reports a TRANSPORT failure, because the port's contract is
 * an outcome, never a checked break. The gateway closes the transport it was handed on
 * {@link #close()}.
 */
public final class BraveHttpImageSearchGateway implements ImageSearchPort, AutoCloseable {
     ...
     */
    public BraveHttpImageSearchGateway(
    ...
    @Override
    public Outcome<ImageSearchResult> search(ImageSearchRequest request) {
        ...
        try {
            exchange = transport.send(
                    ImageSearchEndpoint.assemble(request, origin, credential, pinnedApiVersion), totalTimeout);
    ...
    private static ImageSearchResult project(BraveHttpResponse response) {
