...
import io.amscotti.bravesearch.application.port.out.VideoSearchPort;
...
/**
 * The Brave HTTP gateway behind {@link VideoSearchPort}: one assembled exchange per
 * search.
 *
 ...
 * {@link #close()}.
 */
public final class BraveHttpVideoSearchGateway implements VideoSearchPort, AutoCloseable {
     ...
     */
    public BraveHttpVideoSearchGateway(
    ...
    @Override
    public Outcome<VideoSearchResult> search(VideoSearchRequest request) {
        ...
        try {
            exchange = transport.send(
                    VideoSearchEndpoint.assemble(request, origin, credential, pinnedApiVersion), totalTimeout);
    ...
    private static VideoSearchResult project(BraveHttpResponse response) {
