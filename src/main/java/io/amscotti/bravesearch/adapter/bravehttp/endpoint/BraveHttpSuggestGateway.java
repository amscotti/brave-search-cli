...
import io.amscotti.bravesearch.application.port.out.SuggestPort;
...
/**
 * The Brave HTTP gateway behind {@link SuggestPort}: one assembled GET exchange per
 * suggest lookup.
 *
 ...
 * {@link #close()}.
 */
public final class BraveHttpSuggestGateway implements SuggestPort, AutoCloseable {
     ...
     */
    public BraveHttpSuggestGateway(
    ...
    @Override
    public Outcome<SuggestSearchResult> suggest(SuggestRequest request) {
        ...
        try {
            exchange = transport.send(
                    SuggestEndpoint.assemble(request, origin, credential, pinnedApiVersion), totalTimeout);
    ...
    private static SuggestSearchResult project(BraveHttpResponse response) {
        return new SuggestSearchResult(
                response.statusCode(),
                response.body(),
                response.rateLimits(),
                response.usage(),
