...
import io.amscotti.bravesearch.application.port.out.SpellcheckPort;
...
/**
 * The Brave HTTP gateway behind {@link SpellcheckPort}: one assembled GET exchange per
 * spellcheck lookup.
 *
 ...
 * {@link #close()}.
 */
public final class BraveHttpSpellcheckGateway implements SpellcheckPort, AutoCloseable {
     ...
     */
    public BraveHttpSpellcheckGateway(
    ...
    @Override
    public Outcome<SpellcheckSearchResult> spellcheck(SpellcheckRequest request) {
        ...
        try {
            exchange = transport.send(
                    SpellcheckEndpoint.assemble(request, origin, credential, pinnedApiVersion), totalTimeout);
    ...
    private static SpellcheckSearchResult project(BraveHttpResponse response) {
        return new SpellcheckSearchResult(
                response.statusCode(),
                response.body(),
                response.rateLimits(),
                response.usage(),
