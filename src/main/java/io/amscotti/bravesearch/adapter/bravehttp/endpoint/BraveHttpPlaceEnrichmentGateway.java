...
/**
 * The Brave HTTP gateway behind one place-enrichment chunk exchange: one assembled GET
 * per chunk request, bound by the composition that owns its lifecycle.
 *
 ...
 * per run; the library composition injects the origin's routing rule — the stored production
 * credential, resolved through the injected provider, reaches only the exact production
 * origin, while a loopback origin draws its token from the injected loopback test-token
 * supplier, so a test or proxy override can never capture stored credential material. A
 * credential that cannot resolve on the library path is a local-configuration failure, not
 * an exchange.
 *
 * <p>The exchange travels the shared transport under the injected total budget, and
 * a successful response is projected verbatim: status, lossless body bytes, rate-limit
 * snapshot, usage, and the request and API version identifiers the response headers
 * offered, with no java.net.http type crossing the caller. Failures keep the redacted
 ...
 * caller's contract is an outcome, never a checked break. The gateway closes the
 * transport it was handed on {@link #close()}.
 */
public final class BraveHttpPlaceEnrichmentGateway implements AutoCloseable {
    ...
    /**
     * The library wiring: the credential resolves per fetch through the origin's routing
     * rule between the stored provider and the loopback test-token supplier.
     */
    public BraveHttpPlaceEnrichmentGateway(
    ...
    /**
     * The CLI wiring: the credential the command preflight resolved for this invocation,
     * so the exchange never resolves a second time.
     */
    public BraveHttpPlaceEnrichmentGateway(
            BraveHttpTransport transport,
            BraveApiOrigin origin,
    ...
    /** Performs the enrichment exchange of one chunk request. */
    public Outcome<PlaceEnrichmentResult> fetch(PlaceEnrichmentRequest request) {
        ...
        try {
            exchange = transport.send(
                    PlaceEnrichmentEndpoint.assemble(request, origin, credential, pinnedApiVersion), totalTimeout);
    ...
    private static PlaceEnrichmentResult project(BraveHttpResponse response) {
        return new PlaceEnrichmentResult(
                response.statusCode(),
                response.body(),
                response.rateLimits(),
                response.usage(),
