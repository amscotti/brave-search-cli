...
import io.amscotti.bravesearch.application.port.out.ContextPort;
...
/**
 * The Brave HTTP gateway behind {@link ContextPort}: one assembled exchange per context
 * retrieval — a single non-paginated request, GET unless the encoded-URI rule forces the
 * POST form.
 *
 * <p>The credential arrives through one injected source. The CLI composition hands the
 * gateway the credential its preflight already resolved for the invocation, so resolution
 * happens once per run; the library composition injects the origin's routing rule — the
 * stored production credential, resolved through the injected provider, reaches only the
 * exact production origin, while a loopback origin draws its token from the injected
 * loopback test-token supplier, so a test or proxy override can never capture stored
 * credential material. A credential that cannot resolve on the library path is a
 * local-configuration failure, not an exchange.
 *
 * <p>The exchange travels the shared transport under the injected total budget, and a
 * successful response is projected verbatim: status, lossless body bytes, rate-limit
 * snapshot, usage, and the request and API version identifiers the response headers
 * offered, with no java.net.http type crossing the port. Failures keep the redacted shape
 * the transport and its classifier already produced. A thread interrupted mid-exchange
 * keeps its interrupt status and reports a TRANSPORT failure, because the port's contract
 * is an outcome, never a checked break. The gateway closes the transport it was handed on
 * {@link #close()}.
 */
public final class BraveHttpContextGateway implements ContextPort, AutoCloseable {
    ...
    /**
     * The library wiring: the credential resolves per search through the origin's routing
     * rule between the stored provider and the loopback test-token supplier.
     */
    public BraveHttpContextGateway(
    ...
    @Override
    public Outcome<ContextResult> search(ContextRequest request) {
        ...
        try {
            exchange = transport.send(
                    ContextEndpoint.assemble(request, origin, credential, pinnedApiVersion), totalTimeout);
    ...
    private static ContextResult project(BraveHttpResponse response) {
