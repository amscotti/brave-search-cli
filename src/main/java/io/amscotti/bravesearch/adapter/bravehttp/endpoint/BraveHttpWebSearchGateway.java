...
/**
 * The Brave HTTP gateway behind {@link WebSearchPort}: one assembled GET exchange per
 * search.
 *
 ...
 * {@link #close()}.
 */
public final class BraveHttpWebSearchGateway implements WebSearchPort, AutoCloseable {
     ...
     */
    public BraveHttpWebSearchGateway(
    ...
    @Override
    public Outcome<WebSearchResult> search(WebSearchRequest request) {
        ...
        try {
            exchange = transport.send(
                    WebSearchEndpoint.assemble(request, origin, credential, pinnedApiVersion), totalTimeout);
    ...
    private static WebSearchResult project(BraveHttpResponse response) {
