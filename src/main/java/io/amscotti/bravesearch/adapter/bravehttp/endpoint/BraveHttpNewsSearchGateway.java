...
import io.amscotti.bravesearch.application.port.out.NewsSearchPort;
...
/**
 * The Brave HTTP gateway behind {@link NewsSearchPort}: one assembled GET exchange per
 * search.
 *
 ...
 * {@link #close()}.
 */
public final class BraveHttpNewsSearchGateway implements NewsSearchPort, AutoCloseable {
     ...
     */
    public BraveHttpNewsSearchGateway(
    ...
    @Override
    public Outcome<NewsSearchResult> search(NewsSearchRequest request) {
        ...
        try {
            exchange = transport.send(
                    NewsSearchEndpoint.assemble(request, origin, credential, pinnedApiVersion), totalTimeout);
    ...
    private static NewsSearchResult project(BraveHttpResponse response) {
