...
import io.amscotti.bravesearch.application.port.out.PlaceSearchPort;
...
/**
 * The Brave HTTP gateway behind {@link PlaceSearchPort}: one assembled GET exchange per
 * place search.
 *
 ...
 * {@link #close()}.
 */
public final class BraveHttpPlaceSearchGateway implements PlaceSearchPort, AutoCloseable {
     ...
     */
    public BraveHttpPlaceSearchGateway(
        ...
        this.credentialSource =
                origin.credentialSource(() -> resolveWrapped(storedCredentials), loopbackTestToken);
    ...
    @Override
    public Outcome<PlaceSearchResult> search(PlaceSearchRequest request) {
        ...
        try {
            exchange = transport.send(
                    PlaceSearchEndpoint.assemble(request, origin, credential, pinnedApiVersion), totalTimeout);
    ...
    private static PlaceSearchResult project(BraveHttpResponse response) {
